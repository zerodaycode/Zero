...
export enum TestRunBehavior {
   ...
   * immediately upon encountering a failure, then proceeds to the next suite or
   * free test. It's beneficial for quickly bypassing problematic tests while
   * still executing the remaining tests.
   */
  HALT_SUITE_ON_FAIL,
...
export {
  ...
  struct TestSuite {
     ...
     * file than the main file (in particular from a module), Clang refuses to
     * compile saying that there's no viable ctr for TestSuite receiving a const
     * char*
     *
     * For further @details @see
     * https://github.com/llvm/llvm-project/issues/64211
     */
    constexpr explicit TestSuite(const char *uuid) : uuid(std::move(uuid)) {}
  ...
  struct TestCase {
     ...
     *
     * @details  constexpr constructor's 2nd parameter type 'std::function<void
     * ()>' is not a literal type constexpr TestCase(std::string name,
     * std::function<void()> fn)
     *
     * 'function<void ()>' is not literal because it is not an aggregate and has
     * no constexpr constructors other than copy or move constructors class
     * _LIBCPP_TEMPLATE_VIS function<_Rp(_ArgTypes...)>
     */
...
bool runFreeTestCases(const TestRunBehavior behavior) {
  ...
  if (anyFailed) {
    if (behavior == HALT_SUITE_ON_FAIL) {
      std::cout << "\n\033[1;38;5;214m========================================="
                   "=======\n";
      std::cout << "[Halt Free Tests] Stopping further free tests due to a "
                   "failure.\n";
      std::cout << "================================================\033[0m\n";
    } else if (behavior == ABORT_ALL_ON_FAIL) {
      std::cout << "\n\033[1;38;5;196m========================================="
                   "=======\n";
      std::cout << "[Abort] All further tests are aborted due to a failure in "
                   "free tests.\n";
      std::cout << "================================================\033[0m\n";
