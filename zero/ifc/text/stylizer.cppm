...
export namespace zero::fmt {
    ...
    // TODO: move the impl to a module impl details
    [[nodiscard]] constexpr std::string __stylize_impl(const std::string &text,
                                                       const Color color,
        ...
        if (static_cast<int>(color) >= 30 && static_cast<int>(color) <= 37) {
            ...
            // Colors outside the range of standard ANSI colors are treated as part
            // of the extended 256-color palette. They require a prefix "38;5;"
            // which signals the terminal to interpret the following number as a
            // 256-palette index.
            colorCode =
                "\033[38;5;" + std::to_string(static_cast<int>(color)) + "m";
        }
        ...
        for (const auto &modifier : modifiers) {
            modifierCodes +=
                "\033[" + std::to_string(static_cast<int>(modifier)) + "m";
        }
...
// Only text and color (no modifiers)
[[nodiscard]] constexpr std::string stylize(const std::string &text, const Color color) {
    return __stylize_impl(text, color, {}); // Pass an empty vector for no modifiers
}

// Single modifier overload
[[nodiscard]] constexpr std::string stylize(const std::string &text,
                                            const Color color,
                                            Modifier modifier) {
    return __stylize_impl(text, color, std::vector<Modifier>{modifier});
}

// Multiple modifiers overload
[[nodiscard]] constexpr std::string stylize(const std::string &text,
                                            const Color color,
                                            std::initializer_list<Modifier> modifiers) {
    return __stylize_impl(text, color, std::vector<Modifier>(modifiers));
}
} // namespace zero::fmt
